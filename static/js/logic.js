...
let board = null;
//game variable
//only responsible for move validation, making moves, etc.
let game = new Chess();
...
var objs = [];
//modal for selecting promotion
let $promotionModal = $("#promotionModal");
//configuration for current move 
var moveConfig;
//state of whether or not promotion is currently happening.
var promoting = false;
//location of piece images
// var piece_theme = 'nothing';
var piece_theme = 'static/img/chesspieces/wikipedia/{piece}.png';
...
function onModalClose() {
    //update promotion selection of move configuration
    moveConfig.promotion = promote_to;
    //make move
...
function onDrop (source, target, piece) {
    ...
    if ((/8/.test(target) && /7/.test(source) && /P/.test(piece)) || (/1/.test(target) && /2/.test(source) && /p/.test(piece))) {
        ...
        //create modal to select piece
        $promotionModal.dialog({
            modal: true,
            height: 92,
            width: 92,
            resizable: false,
...
$("#promoteOptions").selectable({
    stop: function() {
      $( ".ui-selected", this ).each(function() {
        var selectable = $('#promoteOptions li');
